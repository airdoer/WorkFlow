...
基于 **React Flow** 开源前端流程搭建引擎，构建一个可视化工作流平台。支持 P4File、Excel、Lua、JSON、Prompt 五类节点，**String / Bool / Number** 三类基础值节点，**C7Server / Jenkins / KimNotify / BoolGate / Diff / Table / ExcelSearch / MergeObject** 八类功能节点，**Cron** 定时触发节点，**SetGlobalValue / GetGlobalValue** 全局存储节点，通过**端口类型系统**实现数据源与渲染器的解耦连接，实现文件获取、解析、AI 处理、服务器操作、通知、定时触发和全局数据存取等操作的流程化编排。
...
└── src/
    ...
    └── components/
        └── workflow/
            ├── FlowEditor.tsx               // ReactFlow 初始化 + 选中节点管理 + handleNodeUpdate + 连线交互（QuickAddMenu/重连/覆盖确认）
            ├── Toolbar.tsx                  // 深色工具栏：名称内联编辑 + 自动保存（800ms防抖） + 工作流库（含搜索/垃圾箱）+ 全屏
            ├── QuickAddMenu.tsx             // 拖出连线新建节点的浮动搜索菜单
            ...
            └── nodes/
                ...
                │
                ├── Diff/
                │   ├── index.tsx             // Diff 对比节点（Monaco DiffEditor）
                │   ├── DiffRenderer.tsx      // Monaco side-by-side diff 渲染器
                │   ├── DiffSummary.tsx       // 轻量摘要组件（卡片/属性面板用）
                │   └── icon.tsx
                │
                ├── Table/
                ...
                │
                ├── MergeObject/
                │   ├── index.tsx             // 合并对象节点（合并两个 dict/list）
                │   └── icon.tsx
                │
                ├── Cron/
...
│       ├── diffExecutor.py                 // Diff 节点执行（difflib 统一差分 + 统计）
│       ├── tableExecutor.py               // Table 节点执行（list/dict → 结构化表格）
│       ├── excelSearchExecutor.py          // ExcelSearch 节点执行（加载文件列表选项）
│       ├── mergeObjectExecutor.py          // MergeObject 节点执行（合并两个 dict/list）
...
2. _to_string 转换：
   - dict 含 localPath → 从磁盘读取文件内容（JSON 文件自动 pretty-print）
   - dict/list → json.dumps(indent=2, ensure_ascii=False)
   - JSON 字符串 → 解析后 pretty-print
   - 其他 → str()
3. 计算 is_same = contentA == contentB
4. 用 difflib.unified_diff 生成 unified diff
5. 统计 additions / deletions 行数
6. 返回:
...
> **P4File 直连 Diff 的数据流：** P4File 执行器输出 `{filePath, localPath, fileType, size}`（不含 `fileContent` 键），当 `sourceHandle='fileContent'` 不在输出中时，运行时引擎将整个输出 dict 赋给 `input_data[targetHandle]`（仅含 `localPath` 的文件引用节点适用此规则），Diff 的 `_to_string` 检测 `localPath` 后直接读取文件内容。
...
### 3.10.1 MergeObject 节点（对象合并）

**设计原则：** MergeObject 接收两个对象（dict/list），执行递归合并，输出合并后的结果。用于合并多个 JSON 配置或上游数据。

**端口定义:**

| 方向 | key | label | type |
|------|-----|-------|------|
| input | objectA | 对象A | any |
| input | objectB | 对象B | any |
| output | merged | 合并结果 | any |

**执行流程（后端）：**

```
1. 从 input_data 获取 objectA、objectB
2. 如果两者都是 dict → 递归合并（objectB 覆盖 objectA 同名键）
3. 如果两者都是 list → 拼接
4. 其他 → 尝试合并，不兼容时返回 objectB
5. 返回 { merged: 合并后的对象 }
```
...
| MergeObject | objectA (any), objectB (any) | merged (any) |
...
### 5.5.1 拖出连线新建节点（QuickAddMenu）

**交互流程：**

1. 用户从节点输出端口（source Handle）拖出连线
2. 松开鼠标时，如果连线**未落到**任何目标 Handle 上：
   - 在鼠标释放位置弹出 `QuickAddMenu`（深色主题浮动搜索菜单）
   - 菜单显示所有可用节点类型，支持关键词搜索
   - 键盘 ↑↓ 导航、Enter 确认、Escape 关闭
3. 用户选择节点类型后：
   - 在鼠标释放位置创建新节点
   - 自动用 `sourceHandle → targetHandle` 连线
   - 端口类型兼容性检查，不兼容时显示 warning
4. 如果连线**落到**了已有 Handle 上：直接连接（不弹菜单）
5. 点击画布空白区域：关闭菜单
...
interface QuickAddMenuProps {
  canvasPosition: { x: number; y: number };   // 画布坐标（用于新节点定位）
  sourceId: string;                            // 拖出连线的源节点 ID
  sourceHandle: string;                        // 源端口 key
  sourcePortType?: string;                     // 源端口类型（用于兼容性筛选）
  onSelect: (nodeType: string, targetHandle: string) => void;
  onClose: () => void;
  style?: React.CSSProperties;                 // 屏幕坐标定位
}
```

**实现要点：**

- `onConnectStart` 记录源端口信息（nodeId, handleId, type, portType）
- `onConnectEnd` 检查 `connectSucceededRef` 和 `landedOnHandle`，判断是否弹出
- 最小拖拽距离检测（10px），防止误触 Handle 时弹出菜单
- `edgesUpdatable="target"`：只有 target 端显示边重连小圆圈，source 端保留新建连线功能

### 5.5.2 已有连线的重连与解连

**交互流程：**

1. 用户点击已有连线的**目标端**小圆圈（edge updater handle）
2. 拖拽到新节点的输入端口 → 重新连接（`onEdgeUpdate`）
3. 拖拽到空白区域松开 → 删除连线
4. 单击或微小移动（<10px）→ 不删除连线（防止误操作）
5. 重连成功后不会弹出 QuickAddMenu

**实现要点：**

- `edgesUpdatable="target"`：只在 target 端显示重连 handle
- `onEdgeUpdateStart`：记录鼠标起始位置和当前边
- `onEdgeUpdate`：处理重连，检查端口类型兼容性，更新边数据
- `onEdgeUpdateEnd`：检查拖拽距离 + 是否成功，决定是否删除边
...
1. 用户拖出连线到已连接的输入端口
2. 弹出 `Modal.confirm`："端口已有连线，是否替换原有连线？"
3. 用户选择「替换」→ 删除旧边，创建新边，清除下游运行状态
4. 用户选择「保留原连线」→ 不做任何操作
5. QuickAddMenu 不会弹出（因为 `onConnect` 已被调用）

**实现要点：**

- `onConnect` 中检测 `edges.find(target === params.target && targetHandle === params.targetHandle)`
- 存在已有边时弹 `Modal.confirm`
- 替换后 BFS 清除目标节点及所有下游节点的运行状态
...
从 Source Handle 拖出
    │
    ├── 落到 Target Handle ──→ 直接连接
    │   └── 已有连线? ──→ Modal.confirm 覆盖确认
    │
    └── 未落到 Handle ──→ 拖拽距离 ≥ 10px?
        ├── 是 ──→ 弹出 QuickAddMenu
        └── 否 ──→ 忽略（误触）

已有连线 Target 端重连
    │
    ├── 拖到新 Handle ──→ onEdgeUpdate 重连
    │
    └── 拖到空白区域 ──→ 拖拽距离 ≥ 10px?
        ├── 是 ──→ 删除连线
        └── 否 ──→ 保留连线（误触）
...
| Diff | 🔍 | 工具 | 无配置字段，完全由连线提供 contentA/contentB |
| MergeObject | 🔗 | 工具 | 无配置字段，完全由连线提供 objectA/objectB |
...
export const nodeRegistryList: NodeRegistryEntry[] = [
  ...
  { type: 'table', label: 'Table', icon: <TableIcon />, category: '渲染器' },
  { type: 'excelsearch', label: 'Excel 搜索', icon: <ExcelSearchIcon />, category: '数据源' },
  { type: 'mergeobject', label: '合并对象', icon: <MergeObjectIcon />, category: '工具' },
  { type: 'cron', label: 'Cron 定时', icon: <CronIcon />, category: '触发器' },
  { type: 'setglobalvalue', label: '设置全局值', icon: <SetGlobalValueIcon />, category: '全局存储' },
  { type: 'getglobalvalue', label: '获取全局值', icon: <GetGlobalValueIcon />, category: '全局存储' },
];
```
...
**端口映射规则（`execute_node` 中 `input_data` 组装）：**

| 条件 | 行为 | 说明 |
|------|------|------|
| `source_handle in src_output` | `input_data[target_handle] = src_output[source_handle]` | 精确映射，port key 与 output key 一致 |
| `source_handle not in src_output` 且 `__value__ in src_output` | `input_data[target_handle] = src_output['__value__']` | fallback：用 `__value__` 通用输出 |
| `source_handle not in src_output` 且 `localPath in src_output` | `input_data[target_handle] = src_output`（整个 dict） | 文件引用节点（P4File）port key 被移除，传整个元数据 |
| `source_handle not in src_output` 且无上述条件 | key-level merge：`input_data[k] = v`（跳过 meta key） | 最后 fallback：按 key 名合并 |
| 无 `target_handle` | `input_data[target_handle] = src_output`（整个 dict） | 无 handle 信息时整体赋值 |
...
2. ~~运行历史记录~~ — 未实现
3. ~~错误处理与重试~~ — 未实现
4. ~~Undo / Redo 支持~~ ✅（pushUndo + Ctrl+Z）

### Phase 11: 连线交互增强 ✅（已完成）

1. ~~拖出连线到空白区域 → 弹出 QuickAddMenu（搜索/键盘导航/自动连线）~~ ✅
2. ~~已有连线 target 端重连（onEdgeUpdate + edgesUpdatable="target"）~~ ✅
3. ~~拖到空白区域删除连线 + 最小拖拽距离防误触~~ ✅
4. ~~输入端口覆盖确认（Modal.confirm）~~ ✅
5. ~~点击画布关闭 QuickAddMenu~~ ✅
6. ~~Toolbar 元信息（author/description）自动保存（800ms 防抖）~~ ✅

### Phase 12: 存储优化 + 新节点 ✅（已完成）

1. ~~归档机制：大输出写入磁盘，JSON 中保留引用标记~~ ✅
2. ~~源节点去掉 fileContent 输出，下游用 localPath 读取~~ ✅
3. ~~MergeObject 节点（合并两个 dict/list）~~ ✅
4. ~~Diff 节点 P4File 直连修复（端口映射 + _to_string 支持 dict/localPath）~~ ✅
5. ~~Excel 处理 dict 格式 fileContent（从 dict 提取 localPath）~~ ✅
6. ~~Toolbar 工作流库显示 name(description) 格式~~ ✅
7. ~~节点编号（_seq）拓扑排序 + SeqBadge 显示~~ ✅
