...
def _visitor_impl(self, arg):
    ...
    # Search for visitor method in the class hierarchy of self
    for cls in type(self).__mro__:
        if cls is object:
            continue
        
        cls_name = _qualname(cls)
        
        # Check if a visitor is registered for this visitor class and arg type
        if (cls_name, type(arg)) in _methods:
            method = _methods[(cls_name, type(arg))]
        ...
        while arg_parent_type != object:
            if (cls_name, arg_parent_type) in _methods:
                method = _methods[(cls_name, arg_parent_type)]
                return method(self, arg)
            else:
